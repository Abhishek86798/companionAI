...
| Payments | Razorpay (JS SDK on frontend, Python SDK on backend) |
...
- **Vite** as the build tool (not Create React App — CRA is deprecated)
...
| `react-hot-toast` | ^2.x | Toast notifications (paywall nudge, errors) |
...
│   └── payments.ts       # createOrder, verifyPayment
├── components/
│   ├── chat/
│   │   ├── ChatBubble.tsx
│   │   ├── ChatInput.tsx
...
- Constants: `SCREAMING_SNAKE_CASE` (`MAX_FREE_MESSAGES = 5`)
...
│
├── routers/
│   ├── chat.py              # POST /message, GET /messages/{conversation_id}
│   ├── memories.py          # GET /memories, DELETE /memories/{id}
│   ├── auth.py              # POST /auth/otp/send, POST /auth/otp/verify
│   ├── payments.py          # POST /payments/order, POST /webhook/razorpay
│   ├── transcribe.py        # POST /transcribe (future Bhashini; passthrough for MVP)
│   └── notifications.py     # POST /notifications/subscribe, cron trigger
│
├── services/
│   ├── ai.py                # OpenAI API call, prompt assembly
│   ├── memory.py            # Memory extractor, summarizer, prompt injector
│   ├── safety.py            # Keyword check + semantic classification
│   ├── rate_limiter.py      # Daily message counter logic
...
| POST | `/api/v1/auth/otp/send` | ❌ | Trigger Supabase OTP (phone or email) |
| POST | `/api/v1/auth/otp/verify` | ❌ | Verify OTP → return JWT |
| POST | `/api/v1/payments/order` | ✅ | Create Razorpay order |
| POST | `/api/v1/webhook/razorpay` | ❌ (HMAC) | Payment webhook → upgrade tier |
...
2. If count >= 5 AND user.tier == 'free' → raise HTTP 429 with paywall message
3. If Plus user → skip check entirely
4. After successful AI response → UPDATE daily_usage SET msg_count = msg_count + 1
...
| Razorpay | Payments (UPI + cards) | `razorpay` (Python), Razorpay JS | Free account + 2% per transaction | `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_WEBHOOK_SECRET` |
...
### 8.2 Frontend `.env` (Vite — prefix all with `VITE_`)
```bash
...
> ⚠️ **Never commit `.env` files.** Add to `.gitignore`. Set all vars in Vercel and Railway dashboards.
...
### 9.4 Streaming (SSE)
- FastAPI returns AI responses as **Server-Sent Events** (not a single JSON blob)
- Frontend reads the stream and renders tokens progressively (like ChatGPT)
- Endpoint: `POST /api/v1/message` returns `text/event-stream`
- Fallback: if streaming fails, return full response as JSON (graceful degradation)
