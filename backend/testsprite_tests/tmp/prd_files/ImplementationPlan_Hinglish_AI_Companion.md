...
- [ ] Create folder structure exactly as defined in TRD Section 3.2:
  ...
  app/routers/ (empty files: chat.py, memories.py, auth.py, notifications.py, transcribe.py)
  app/services/ (empty files: ai.py, memory.py, safety.py, rate_limiter.py, push.py)
...
- [ ] Implement `app/dependencies.py` — `get_current_user()` function that decodes and verifies Supabase JWTs (see BackendSchema Section 6.4)
- [ ] Implement `app/routers/auth.py`:
  - `POST /api/v1/auth/otp/send` — calls Supabase Auth to send OTP (phone or email)
  - `POST /api/v1/auth/otp/verify` — verifies OTP; on success, creates `public.users` row if first login
- [ ] Register auth router in `app/main.py`
- [ ] Test both routes manually with `curl` or Bruno/Postman
...
- [ ] Implement `app/services/rate_limiter.py`:
  - `check_and_increment(user_id) → None` — raises `HTTP 429` if `msg_count >= 5` for today
  - Upserts to `public.daily_usage` after successful AI response
...
- Rate limiter blocks the 6th message and shows the limit-reached message
...
- [ ] Update `VITE_API_BASE_URL` to Railway production URL
